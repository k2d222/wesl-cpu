...
function getStaticUsages(wesl: any, entryPoint: Object): any[] {
	...
	for (const decl of wesl.global_declarations) {
		if (decl.Declaration) {
			if (
				decl.Declaration.attributes.findIndex((attr: any) => attr.Group) !== -1
			) {
				res.push(decl.Declaration);
			}
		}
	}
...
function createDefaultPipelineLayout(device: CPUDevice, code: string) {
	...
	for (const resource of staticUsages) {
		const group = Number(
			resource.attributes.find((attr: any) => attr.Group).Group.node.Literal
				.AbstractInt,
		);
		const binding = Number(
			resource.attributes.find((attr: any) => attr.Binding).Binding.node.Literal
				.AbstractInt,
		);
